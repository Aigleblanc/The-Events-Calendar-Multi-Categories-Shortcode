...
class Tribe__Events__Pro__Shortcodes__Tribe_Events {
	...
	public function prepare_query() {
		...
		*/
		...
		/* -- Code who add multi cat in shortcode */
		...
		$rq['tax_query'] 	= array('relation' => 'OR');
		
		$categories = explode(',', $this->atts[ 'category' ]);

		foreach($categories as $cat){
			$array_cat = array(
				'taxonomy' 	=> 'tribe_events_cat', 
				'field' 		=> 'slug',
				'terms' 		=> esc_attr(trim($cat))
			);

			array_push($rq['tax_query'], $array_cat);
		}

		/* -- End Code who add multi cat in shortcode */

		$this->update_query( $rq );
